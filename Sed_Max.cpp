...
void merge(vll &A, int l, int m, int r, set<ll> &ans)
{
    int n1 = m - l + 1;
    int n2 = r - m;
    vector<int> L(A.begin() + l, A.begin() + m + 1), R(A.begin() + m + 1, A.begin() + r + 1);
    int i = 0, j = 0, k = l;
    int maxx1=INT_MIN, maxx2=INT_MIN, maxx3=INT_MIN, maxx4=INT_MIN;
    if(n1>1){
        maxx2=L[n1-2];
    }
    maxx1=L[n1-1];
    ...
    maxx3=R[n2-1];
    vector<int> v={maxx1,maxx2,maxx3,maxx4};
    sort(all(v));
    ans.insert(v[3]-v[2]);
    while (i < n1 && j < n2)
    {
...
void mergeSort(vll &A, int l, int r, set<ll> &ans)
{
    if (l >= r)
        return;
    int m = (l + r) / 2;
